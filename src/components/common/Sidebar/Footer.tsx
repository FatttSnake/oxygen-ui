...
const Footer = ({ isCollapse = false }: FooterProps) => {
    ...
    return (
        <div className={styles.footer}>
            ...
            <span
                ...
                className={`${styles.text}${isCollapse ? ` ${styles.collapsedText}` : ''}`}
                title={nickname}
            >
                {nickname}
            </span>
            <div
                hidden={!getLoginStatus()}
                className={`${isCollapse ? styles.collapsedExit : ''}${!getLoginStatus() ? ` ${styles.hide}` : ''}`}
            >
                <div
                    className={`${styles.exitContent}${isCollapse ? ` ${styles.collapsedExitContent}` : ''}`}
                >
                    <span
                        hidden={!getLoginStatus()}
                        className={`${styles.exitIcon}${isCollapse ? ` ${styles.collapsedExitIcon}` : ''}`}
